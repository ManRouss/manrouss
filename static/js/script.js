...
$(document).ready(function() {
		$('#tweets-sidebar').jtwt({image_size : 0, count : 3, username: 'ManRouss', convert_links : 1, loader_text : 'Loading new tweets...'});   
});
...
/*$(document).ready(function(){		 			   
	...
	$.getJSON("http://api.flickr.com/services/feeds/photos_public.gne?id=&lang=en-us&format=json&jsoncallback=?", displayImages);  //YOUR IDGETTR GOES HERE
...
function checkFields(obj){
	...
	var nom = $('#contactname').val();
	var email = $('#email').val();
	var men = $('#message').val();
	if(nom == '' || email == '' || men == ''){
		alert('Coloca todos los datos por favor.');
		return false;
	}
	return true;
}
